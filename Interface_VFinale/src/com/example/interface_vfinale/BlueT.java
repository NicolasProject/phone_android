...
public class BlueT
{
	...
	public BlueT(Activity Activity, Handler Handler)
	{
		...
		mThreadReception = new Thread(new Runnable() { // création du thread de
														// réception
					@Override
					public void run()
					{
	...
	{
		firstConnection = true; // on doit selectionner le device avant de pouvoir appeler tryconnect() dans les autres
								// methodes (ecrire, lecture)
		...
		adBuilder.setSingleChoiceItems(mstrDeviceName, 0,
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						mDeviceSelected = which;
						dialog.dismiss();
						tryconnect(); // essai de connection avec le device
										// cible
					}
				});
	...
	public void Device_Connu() // récuperation des mDevices déjà connus par le
								// tel
	{
	...
	{		
		...
		if (firstConnection)
		{
			...
			{
				Toast.makeText(this.mActivity, "Error I/O streams", Toast.LENGTH_SHORT).show();
			}
	...
	public boolean envoi(String strOrdre) // false -> erreur; true -> ok
	{
		...
		do
		{
			...
			catch (Exception e2)
			{
				...
				iCptCrash++;
				/*
				 * try { this.mSocket.close(); } catch (IOException e) { // TODO
				 * Auto-generated catch block e.printStackTrace(); }
				 * //this.mbtConnected = false;
				 */
			}
		} while (mbtConnected && iCptCrash < 2 && bRet == false);
	...
	private String reception()
	{
		...
		do
		{
			...
			catch (Exception e)
			{
				...
				iCptCrash++;
				/*
				 * Log.i(TAG, "erreur"); try { mSocket.close(); } catch
				 * (IOException e1) { // TODO Auto-generated catch block
				 * e1.printStackTrace(); } //this.mbtConnected = false;
				 */
			}
		} while (mbtConnected && iCptCrash < 2 && success == false);
	...
	public void destructor()
	{
		...
		mbtConnected = false;
		//resetConnection(); // pose des problemes ... se reconnect tout seul ! -> exceptions
		mbtAdapt.disable();
	}
